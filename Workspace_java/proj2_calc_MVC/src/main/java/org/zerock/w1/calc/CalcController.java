...
public class CalcController extends HttpServlet {
	...
	if(chk1s!=null) {for(int i=0; i<chk1s.length; i++) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = 0;
		try { result=Integer.parseInt(num1)+Integer.parseInt(num2);
		}catch(Exception e) {
			e.printStackTrace();
		}
		//절대주소
		// response.sendRedirect("/proj2_calc_MVC/calc/calcResult.jsp?result="+result);
		// 상대주소
